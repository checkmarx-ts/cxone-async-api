...
async def generate_upload_link(client: CxOneClient) -> requests.Response:
    """|LowLevelApiDocstring| Verb: POST Endpoint: /api/uploads"""
    url = urljoin(client.api_endpoint, "uploads")
    return await client.exec_request(requests.post, url)


async def upload_to_link(
    client: CxOneClient, upload_link: str, local_zip_path: str
) -> requests.Response:
    """
    ...
    with open(local_zip_path, "rb") as zip_to_upload:
        upload_response = await client.exec_request(
            requests.put, upload_link, data=zip_to_upload
        )

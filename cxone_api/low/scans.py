...
async def run_a_scan(client: CxOneClient, payload: dict) -> requests.Response:
    """|LowLevelApiDocstring| Verb: POST Endpoint: /api/scans"""
...
async def retrieve_list_of_scans(client: CxOneClient, **kwargs) -> requests.Response:
    """|LowLevelApiDocstring| Verb: GET Endpoint: /api/scans"""
    url = urljoin(client.api_endpoint, "scans")
...
async def delete_a_scan(client: CxOneClient, scanid: str) -> requests.Response:
    """|LowLevelApiDocstring| Verb: DELETE Endpoint: /api/scans/{scanid}"""
    url = urljoin(client.api_endpoint, f"scans/{scanid}")
...
async def retrieve_scan_workflow(client: CxOneClient, scanid: str, **kwargs):
    """|LowLevelApiDocstring| Verb: GET Endpoint: /api/scans/{scanid}/workflow"""
...
async def retrieve_scan_status_summary(client: CxOneClient) -> requests.Response:
    """|LowLevelApiDocstring| Verb: GET Endpoint: /api/scans/summary"""
    url = urljoin(client.api_endpoint, "scans/summary")
    return await client.exec_request(requests.get, url)


async def retrieve_templates(client: CxOneClient) -> requests.Response:
    """|LowLevelApiDocstring| Verb: GET Endpoint: /api/scans/templates"""
    url = urljoin(client.api_endpoint, "scans/templates")
    return await client.exec_request(requests.get, url)


async def retrieve_template_file(
    client: CxOneClient, filename: str
) -> requests.Response:
    """|LowLevelApiDocstring| Verb: GET Endpoint: /api/scans/templates/{filename}"""
    url = urljoin(client.api_endpoint, f"scans/templates/{filename}")
    return await client.exec_request(requests.get, url)


async def retrieve_tags(client: CxOneClient) -> requests.Response:
    """|LowLevelApiDocstring| Verb: GET Endpoint: /api/scans/tags"""
    url = urljoin(client.api_endpoint, "scans/tags")
    return await client.exec_request(requests.get, url)


async def update_scan_tags(
    client: CxOneClient, scan_id: str, payload: dict
) -> requests.Response:
    """|LowLevelApiDocstring| Verb: PUT Endpoint: /api/scans/{scan_id}/tags"""
    url = urljoin(client.api_endpoint, f"scans/{scan_id}/tags")
    return await client.exec_request(requests.put, url, json=payload)


async def retrieve_values_for_fields(
    client: CxOneClient, **kwargs
) -> requests.Response:
    """|LowLevelApiDocstring| Verb: GET Endpoint: /api/scans/fieldValues"""
...
async def run_a_repo_scan(
    client: CxOneClient, scmid: str, projectId: str, repo_org: str, payload: dict
) -> requests.Response:
    """|LowLevelApiDocstring| Verb: POST Endpoint: /api/repos-manager/scms/{scmid}/orgs/{repo_org}/repo/projectScan?projectId={projectId}"""
    url = urljoin(
        client.api_endpoint,
        f"repos-manager/scms/{scmid}/orgs/{repo_org}/repo/projectScan?projectId={projectId}",
    )
    return await client.exec_request(requests.post, url, json=payload)

...
from ..low.scans import retrieve_scan_details, run_a_scan, run_a_repo_scan
    ...
    async def scan_get_response(cxone_client : CxOneClient, project_repo : ProjectRepoConfig,
        ...
        if (not await project_repo.is_scm_imported) or (src_zip_path is not None) or \
            ...
            return await run_a_repo_scan(cxone_client, await project_repo.scm_id,
                project_repo.project_id, scm_org
                if scm_org is not None else "anyorg", submit_payload)
...
class ScanInspector:
    ...
    def executing(self):
        if self.__root_status() in ScanInspector.__executing_states:
            ...
            return len([s for s in self.__current_engine_states()
              if s in ScanInspector.__executing_states + ScanInspector.__maybe_states]) > 0
